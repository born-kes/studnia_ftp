...
if($_GET[q]!=NULL){$url = url_proxi($_GET[q]);} //foreach($url as $u => $l){ echo $u.', '.$l.', '.$u[$l].'<BR>';}
//*
$kolor = ImageColorAllocate($dest,103,130,250);
//ImageString($dest, 0, 1, 3, $url['id'],$kolor);
...
LEFT JOIN list_ataki a ON w.id = a.cel AND a.godz>$ec
  WHERE w.id='".$url['id']."' ORDER BY `a`.`godz` ASC ;";//LIMIT 1;";
//echo $zapytanie;/*
connection();
