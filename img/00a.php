...
$szp=-5;for($i=0 ;$i<$s+2; $i++){ImageRectangle($obrazek ,$szp,0,$szp,$sz,$kolor); $szp+=$wi;}    //Pionowe
$szp=-5;for($i=0 ;$i<$s+2; $i++){ImageRectangle($obrazek ,-1,$szp,$sz+1,$szp,$kolor); $szp+=$wi;} //Poziome
}
$wies = ImageCreate($wi-1,$wi-1);

 function xy($x,$y) { global $wi;   $x=($x*$wi)-4;   $y=($y*$wi)-4;   return array ($x,$y); }

$oko = intval($s/2);
$xy = explode("|", $_GET[xy]);
$x1=$xy[0]-$oko-1; $x2=$xy[0]+$oko;
$y1=$xy[1]-$oko-1; $y2=$xy[1]+$oko;
