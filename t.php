...
if($_POST[id]!==NULL)
{
    ...
    $zap="UPDATE proxi SET link='$pos_link',status='$pos_status',ip='$pos_ip' Where id='$pos_id'" ;
//echo $zap;
connection();if(mysql_query($zap)){echo 'zapisabo zmiany';}  destructor();
 ...
 $zap="SELECT id,name,link,status,ip FROM `proxi` Where id=".$_GET[g];
//echo $zap;
connection();$wynik = mysql_query($zap);
 ...
 //efekt
  if($r = @mysql_fetch_array($wynik))
  {
      ...
      echo 'link:<input type="text" name="link" value="'.$r[link].'" /><br>';
...
<?PHP
# $zap="SELECT id,name,link,status,ip FROM `proxi`";connection();$wynik = mysql_query($zap);
#echo "<table>";  while($r = @mysql_fetch_array($wynik))
#  {echo "<tr><td>$r[id]</td><td>$r[name]</td><td>$r[link]</td><td>$r[ip]</td></tr>";}echo "</table>"; 
?>