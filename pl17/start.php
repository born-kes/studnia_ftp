...
if($_GET[t]==null)
    {
     $zap = "SELECT player FROM `ws_all17` Where id='".$_GET[village]."';";//wioska

connection();$wynik = @mysql_query($zap);$id_player = mysql_fetch_array($wynik);destructor();
if($id_player[0]==null){echo ' Error Wioski niema w Bazie Danych dlatego nie wiem kim jeste¶ .' ;exit();}
     echo '<input type="hidden" name="gracz" value="'.$id_player[0].'" />';
     }
else
...
<img src="../img/cal.gif" border="0" height="16" width="16" alt="kliknij Tu by ustaliæ Datê"></a>
</td>
</tr>
<tr><td valign="top"><b>Wojska maja dotrzec na podana godzine?</b><br />
<input name="on" value="0" type="radio" checked="tak">Nie, pokaz tez te które dojda wczesniej <br />
<input name="on" value="1" type="radio">Tak, pomin te które beda za wczesnie<br />
